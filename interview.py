...
try:
    import registry  # the real module, when present
except ImportError:
    import interview_registry_stub as registry

try:
    from registry import ensure_entity_columns
except ImportError:
    from interview_registry_stub import ensure_entity_columns
...
def _ratify_new(conn, name: str, etype: str, aliases: list, vault_dir) -> str | None:
    ...
    existing = registry.match(conn, name, etype)
    if existing:
        _merge_aliases(conn, existing, aliases, vault_dir)
...
def commit_checkin(conn, data: dict, vault_dir) -> list[str]:
    ...
    for u in data.get("project_updates", []):
        eid = registry.match(conn, u["name"], "project")
        if not eid:
            log.warning("interview[ci]: status update for unknown project %r skipped", u["name"])
            continue
        row = conn.execute("SELECT note_path FROM entities WHERE entity_id=?", (eid,)).fetchone()
