...
| `selftest.py` | Preflights the whole pipeline against real models. Twenty-nine gates covering dependencies, retrieval, grounding, citation integrity, the front door, the registry/governance contract (including rejection permanence and rollup deletion staying deleted), the affect-prohibition on distillation, the protocol surface, a frozen-prompt speed benchmark, and the ratification-burden reading |
